...
const ThemeDialog = ({ open, onOpenChange }: ThemeDialogProps) => (
  <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle className="text-3xl font-bold text-center mb-6 font-devanagari">
          सेवा संगठन परिवर्तन
        </DialogTitle>
      </DialogHeader>
      <div className="space-y-6">
        {themes.map((t) => (
          ...
          >
            <h3 className="text-2xl font-bold text-primary mb-4 font-devanagari">{t.title}</h3>
            <p className="text-foreground leading-relaxed mb-4 font-devanagari text-[15px]">
              {t.description}
            </p>
            <p className="text-primary font-semibold italic font-devanagari">{t.tagline}</p>
          </div>
        ))}
      </div>
    </DialogContent>
